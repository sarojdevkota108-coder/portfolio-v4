...
function SectionHeader({ title, sub }: { title: string; sub: string }) {
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <h2 style={{ fontSize: 28, fontWeight: 700, color: '#fff', letterSpacing: '.04em' }}>{title}</h2>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <Card>
          <div style={{ fontSize: 12, color: '#8899bb', fontFamily: 'monospace', marginBottom: 16 }}>
            {editing ? '✏️  EDITING' : '＋ ADD CERTIFICATION'}
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
            <Btn onClick={save} variant="primary">{editing ? 'Update' : 'Add Cert'}</Btn>
            {editing && <Btn onClick={() => { setEditing(null); setForm({ color: 'blue', status: 'done' }) }} variant="ghost">Cancel</Btn>}
          </div>
        </Card>

        <div style={{ maxHeight: 520, overflowY: 'auto' }}>
          {items.map(item => (
            <Card key={item.id} style={{ marginBottom: 10, borderLeft: `3px solid ${COLOR_DOT[item.color] || '#4f7fff'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#e0e8ff' }}>{item.name}</div>
                  <div style={{ fontSize: 11, color: '#8899bb', fontFamily: 'monospace', marginTop: 2 }}>
                    {item.issuer} · {item.year}
                  </div>
                  <span style={{
                    display: 'inline-block', marginTop: 6, fontSize: 10, fontFamily: 'monospace',
                    color: STATUS_COLORS[item.status], letterSpacing: '.06em',
                  }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div style={{ maxHeight: 520, overflowY: 'auto' }}>
          {items.map(item => (
            <Card key={item.id} style={{ marginBottom: 10, borderLeft: `3px solid ${COLOR_DOT[item.color] || '#00d4ff'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  ...
                  {item.tag && <div style={{ fontSize: 11, color: COLOR_DOT[item.color], fontFamily: 'monospace', marginTop: 4 }}>{item.tag}</div>}
                </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '0 20px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>CMS Admin</div>
        </div>
